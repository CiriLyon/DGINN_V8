...
server = function(input , output , session){
  ...
  observeEvent(input$soumettre,{
    ...
    if(!is.null(input$covrageUpload)) {
      couverture <- read.delim(input$covrageUpload$datapath, sep = "\t" , header = F , row.names = 2 )
    ...
    if(is.null(monDoc)) {
      showModal(
        modalDialog(
          title = "No data were uploaded yet",
          "Upload your data file before you press 'Submit' button!",
    ...
    obligatoyCol <- c('FullName' , 'Gene', 'GeneSize', 'Omega', 'Method', 'PosSel', 'NbSites' , 'PSS')
    ...
    #-- Message en cas de doublon dans Gene par Method.
    if(max(table(leaTab$Gene  , leaTab$Method)) > 1){
      showModal(
        modalDialog(
          title = "Invalid file contents!",
          paste("To produce figures, names in the Gene column cannot be repeated between different sets of results. Gene by Methode must be unique !"),
          #footer = modalButton("OK"),
          size = "m",
          easyClose = T, 
          fade = T
    ...
    if(identical(rownames(sumSiteByGene) , checksiteNbr[,1])){
      if(!!sum(sumSiteByGene[,1] - checksiteNbr[,2])){
    ...
    posX <-  bornSup + 0.15 * bornSup #-1#-0.09
    
    text(rep(posX, length(xx)) , xx,  tabFin_v2$leaTab.Omega , cex = max(0.1, min(15/nrow(tabHeat), 1.5)))
    
    par(mar = rep(0,4), xpd  = T)
    
    text(   posX , max(xx) + 2.5 , expression(paste(omega)), cex = min(80/nrow(tabHeat), 1.5))
    ...
    for(t in 1:length(lstGene)){
      ...
      for(CC in 1:ncol(X5)){
        ...
        if(CC == 1){
          ...
          if (exists("couverture")) {
          
            par(xpd = T)
            
            nomComplet <- #dans le fichier de la couverture on n'a que le nom complet du gène, ce qui correspond à la colonne "FullName".
              as.character(unique(leaTab[leaTab$Gene == names(lstGene)[t] , "FullName"]))
            
            if (nomComplet %in% rownames(couverture)) { #la couverture peut être présente pour certains gènes seulement.
              genCouvert <- t(couverture[nomComplet , 1:ncol(couverture)])
              valCouvert <- as.numeric(na.omit(genCouvert[, 1]))
              posCouvert <- 1:length(valCouvert)
              
              # print(paste0(nomComplet ,"     ", length(posCouvert)))
              
              
              for (cvrtr in 1:length(valCouvert)) {
                segments(
                  x0   = cvrtr ,
                  y0   = -0.5 ,
                  x1   = cvrtr ,
                  y1   = -(valCouvert[cvrtr] * 2) - 0.5 ,
                  col  = "grey55" ,
                  lwd  = 0.001
                )
              }
          ...
          ####################################### Fin Affichage de la couverture ###################################
      ...
      #-- pour le placement des axes de bas de page: une 1ere fois à PAS - 1, à chaque multiple entier de PAS 
      # en enfin après le tout dernier gène de la liste.
      if(t %in%  c(PAS - 1, (PAS-1)+(1:length(lstGene))*(PAS) , length(lstGene))){ 
        ...
        par(xpd = T)
        axis(side  = 1 , labels = etend , at = etend , cex.axis = 1 , outer = T ,  tick = T , pos = -3 , col ="royalblue", lwd.ticks= 0 ) #uniquement pour afficher les chiffres ,line = -6
        par(xpd = F)
      }
    }#for(t in 1:length(lstGene)){
...
##==========================================================================================================================================================    
  ...
  if (length(lstGene) < 101){  #affichage Web ignoré si le nombre de gène >= 50
    ...
    output$barPlots  = renderPlot({ 
      ...
      #Cex <- log2(tabFin_v2$leaTab.Omega*100)/2
      posX <- bornSup + 0.08*bornSup #-0.0015 #-0.05 #-0.05
        ...
        for(CC in 1:ncol(X5)){
          ...
          if(CC == 1){
            ...
            if (exists("couverture")) {
              ...
              if (nomComplet %in% rownames(couverture)) {
                genCouvert <- t(couverture[nomComplet , 1:ncol(couverture)])
                valCouvert <- as.numeric(na.omit(genCouvert[, 1]))
                posCouvert <- 1:length(valCouvert)
    ...
    showModal(
      modalDialog(
        title = "DGINN's GUI can't display the graphics",
        "Because your data contain more than 100 genes, only the pdf file is available and can be downloaded.",
        footer = modalButton("OK"),
        size = "s" , 
        easyClose = T
      )
    )
  }  #if (length(lstGene) < 100){ 
